...
Этот промпт может запускаться в новой отдельной сессии после всех предыдущих этапов. Сначала восстанови контекст по ТЗ и текущему коду; не предполагай, что предыдущий исполнитель оставил идеальное состояние.
...
- компонент не монтирует скрытые `<video>` для desktop/tablet карточек на mobile.
...
- отображаются только media с `show_on_rueckblick = 1`, `status = 'uploaded'` и `publication_approved_at IS NOT NULL`;
...
- пагинация работает;
- pagination-ссылки сохраняют активные фильтры и меняют только `page`;
...
- `getHomepageMedia` не возвращает media без `publication_approved_at`;
- `getRueckblickMedia` использует `effective_date` с fallback `captured_at -> events.date -> created_at`.
- публичные route-данные не отдают на клиент внутренние поля `object_key`, `thumbnail_object_key`, `publication_approved_*`, `created_by` и служебные publication flags.
